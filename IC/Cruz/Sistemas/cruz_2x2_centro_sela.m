...
function point_res=quarter_poinc2(i, point, t_space, data, A)    
    ...
    if !mod(i,2)
            ...
            C=data{2-mod(i,2), 1}\[0; point];
            x=@(t) [e^(lambda_1*t), e^(lambda_2*t)]*C;
            y=@(t) [data{2-mod(i,2), 1}(2,1)*e^(lambda_1*t), data{2-mod(i,2), 1}(2,2)*e^(lambda_2*t)]*C;
    else
            point_p_b=point+d(1);
